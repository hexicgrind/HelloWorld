...
| File | Use it when |
|---|---|
| `Sotto-1.0.7-arm64.apk` | **Start here.** Every Android phone made in the last several years. 53 MB. |
| `Sotto-1.0.7-universal.apk` | If the arm64 one refuses to install. Works everywhere. 64 MB. |
| `Sotto-1.0.7-arm32.apk` | Only for a genuinely old 32-bit phone. |
...
|---|---|
| "Sotto can't load its on-device model" | Install 1.0.7 or later. Older builds had two separate faults here — 16 KB page alignment, and a code-optimiser bug that broke face detection outright. |
